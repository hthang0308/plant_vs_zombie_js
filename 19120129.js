...
class Plant {
  constructor(
    ...
    this.x = startX + x * width + 30;
    this.y = startY + y * width + 30;
  ...
  draw() {
    ...
    this.plantBody.style.left = this.x + "px";
    this.plantBody.style.top = this.y + "px";
...
function shoot(plant) {
  ...
  if (plant.isActive) {
    ...
    setTimeout(() => {
      if (plant.isActive) {
        new Bullet(plant.x + 38, plant.y + 5, plant.bulletSpeed, plant.damage);
        setTimeout(() => {
